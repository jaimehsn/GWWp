...
User.findByMail = (userMail, result) => {
  sql.query(
    `SELECT * FROM users WHERE email LIKE '${userMail}'`,
    (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(err, null);
...
User.updateByMail = (email, user, result) => {
  sql.query(
    "UPDATE users SET name = ?, lastname = ?, password = ?, phone = ?, category = ? WHERE email LIKE ?",
    [user.name, user.lastname, user.password, user.phone, user.category, email],
    (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
